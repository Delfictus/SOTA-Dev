...
thread_local! {
    /// When set to true, `CudaContext::bind_to_thread()` becomes a no-op.
    /// This is required during CUDA stream capture, where context management
    /// calls (`cuCtxGetCurrent`, `cuCtxSetCurrent`) are illegal.
    /// Set via `set_capture_mode_active()` before entering capture.
    pub(crate) static CAPTURE_MODE_ACTIVE: Cell<bool> = const { Cell::new(false) };
}

/// Enables or disables stream capture mode for the current thread.
///
/// When enabled, `CudaContext::bind_to_thread()` becomes a no-op. This is
/// required because context management API calls are illegal during
/// `cuStreamBeginCapture` / `cuStreamEndCapture`.
///
/// The caller MUST ensure the target context is already bound to the current
/// thread (via a normal `bind_to_thread()` call) BEFORE enabling capture mode.
///
/// # Safety
/// Must be paired: every `set_capture_mode_active(true)` needs a matching
/// `set_capture_mode_active(false)` on the same thread. Leaving capture mode
/// enabled after capture ends will prevent normal context rebinding.
pub fn set_capture_mode_active(active: bool) {
    CAPTURE_MODE_ACTIVE.with(|f| f.set(active));
}
...
impl CudaContext {
    ...
    ///
    /// # Stream Capture Patch
    /// When `CAPTURE_MODE_ACTIVE` is set (via `set_capture_mode_active(true)` before
    /// entering stream capture), this function becomes a no-op. Both `cuCtxGetCurrent`
    /// and `cuCtxSetCurrent` are illegal during stream capture and would return
    /// `CUDA_ERROR_STREAM_CAPTURE_UNSUPPORTED`. The caller is responsible for ensuring
    /// the context is already bound before entering capture mode.
    pub fn bind_to_thread(&self) -> Result<(), DriverError> {
        self.check_err()?;
        if CAPTURE_MODE_ACTIVE.with(|f| f.get()) {
            // Skip context binding during stream capture — the context must
            // already be bound by the caller before entering capture mode.
            return Ok(());
        }
    ...
    ///
    /// # Stream Capture Patch
    /// Returns false during stream capture (when `CAPTURE_MODE_ACTIVE` is set).
    /// This prevents the launch path from pushing cross-stream wait events,
    /// which would trigger `CUDA_ERROR_STREAM_CAPTURE_ISOLATION`.
