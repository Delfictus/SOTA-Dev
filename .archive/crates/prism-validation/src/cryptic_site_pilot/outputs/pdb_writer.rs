...
impl MultiModelPdbWriter {
    ...
    /// Set metadata for Cα-only trajectory
    ///
    /// # Arguments
    ...
    pub fn write<W: Write>(
        ...
        for (model_idx, coords) in conformations.iter().enumerate() {
            ...
            for (atom_idx, coord) in coords.iter().enumerate() {
                ...
                let chain_id = self.chain_ids.get(atom_idx).copied().unwrap_or('A');
                let element = self
                    .elements
                    .get(atom_idx)
                    .map(|s| s.as_str())
                ...
                let bfactor = if self.rmsf_as_bfactor {
                    bfactors
                        .and_then(|b| b.get(atom_idx).copied())
                        .unwrap_or(20.0)
                } else {
                    20.0
                };
    ...
    pub fn write_selected<W: Write>(
        ...
        let selected: Vec<Vec<[f32; 3]>> = frame_indices
            .iter()
    ...
    pub fn write_pocket_focused<W: Write>(
        ...
        writeln!(
            writer,
            "REMARK   Atoms within {:.1} A of pocket centroid",
            radius
        )?;
        writeln!(
            writer,
...
mod tests {
    ...
    fn test_write_single_model() {
        ...
        let coords = vec![vec![[0.0, 0.0, 0.0], [3.8, 0.0, 0.0], [7.6, 0.0, 0.0]]];
    ...
    fn test_write_multi_model() {
        ...
        let conformations: Vec<Vec<[f32; 3]>> = (0..3)
            .map(|m| {
                (0..5)
                    .map(|i| [(i as f32) * 3.8 + (m as f32) * 0.1, 0.0, 0.0])
                    .collect()
            })
            .collect();
    ...
    fn test_selected_frames() {
        ...
        let conformations: Vec<Vec<[f32; 3]>> = (0..10).map(|_| vec![[0.0, 0.0, 0.0]; 3]).collect();

        let selected_indices = vec![0, 5, 9];

        let mut output = Vec::new();
        writer
            .write_selected(&conformations, &selected_indices, &mut output, None)
            .unwrap();
