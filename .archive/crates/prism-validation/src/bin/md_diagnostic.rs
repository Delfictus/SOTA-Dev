...
fn analyze_structure(name: &str, topo: &Topology) {
    ...
    println!(
        "  Mass range: {:.2} - {:.2} (avg {:.2})",
        min_mass, max_mass, avg_mass
    );
    ...
    println!(
        "  Avg LJ sigma: {:.3} Å, epsilon: {:.4} kcal/mol",
        avg_sigma, avg_eps
    );
    ...
    for i in 0..topo.n_atoms.min(1000) {
        // Sample first 1000 atoms
    ...
    println!(
        "  Close contacts (<2Å): {}, Very close (<1.5Å): {}",
        close_contacts, very_close
    );
}
