...
fn run_comprehensive_md(
    ...
    let mut batch =
        AmberSimdBatch::new_with_config(context, max_atoms, 1, OptimizationConfig::default())
    ...
    let (pe, ke, temp) = if let Some(r) = results.first() {
        ...
        println!(
            "  ├─ Total E: {:.1} kcal/mol",
            r.potential_energy + r.kinetic_energy
        );
        println!(
            "  ├─ Temperature: {:.1} K (target: {})",
            r.temperature, temp_target
        );
        println!(
            "  └─ KE ratio: {:.1}% of expected {:.0}",
            100.0 * r.kinetic_energy / expected_ke,
            expected_ke
        );
...
fn main() -> Result<()> {
    ...
    let structures = [
        (
            ...
            "results/prism_prep_test/1L2Y_topology.json",
        ),
        (
            "1HXY (Hemoglobin, 585 res)",
            "results/prism_prep_test/1HXY_apo_topology.json",
        ),
        (
            "2VWD (Nipah G, 823 res)",
            "results/prism_prep_test/2VWD_apo_topology.json",
        ),
        (
            "6M0J (SARS-CoV-2 RBD, 791 res)",
            "results/prism_prep_test/6M0J_apo_topology.json",
        ),
    ];
    ...
    println!(
        "║ {:^30} │ {:>12} │ {:>12} │ {:>8} ║",
        "Structure", "PE (kcal/mol)", "KE (kcal/mol)", "Temp (K)"
    );
    println!("╟────────────────────────────────┼──────────────┼──────────────┼──────────╢");
    ...
    for (name, pe, ke, temp, ok) in &results {
        if *ok {
            ...
            let temp_status = if *temp > 250.0 && *temp < 350.0 {
                "✓"
            } else {
                "⚠"
            };
    ...
    let all_temp_normal = results
        .iter()
        .filter(|r| r.4)
        .all(|r| r.3 > 250.0 && r.3 < 350.0);
    ...
    if all_negative_pe && all_temp_normal {
        ...
        if !all_negative_pe {
            println!(
                "║  ⚠ Some structures have positive PE - may indicate structural issues       ║"
            );
        }
        if !all_temp_normal {
            println!(
                "║  ⚠ Some structures have abnormal temperatures - thermostat issues          ║"
            );
        }
    }
