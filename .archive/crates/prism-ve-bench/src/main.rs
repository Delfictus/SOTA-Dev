...
use gpu_benchmark::{
    get_lineage_epitope_escape_scores, get_lineage_escape_score, get_lineage_transmissibility,
    init_dms_escape_per_epitope, init_dms_escape_scores, init_mutations_cache,
    init_reference_structure, load_variant_structure, VariantStructure,
};
use immunity_dynamics::ImmunityDynamics;
use immunity_model::{CrossReactivityMatrix, PopulationImmunityLandscape};
use prism_4d_forward_sim::StructuralNeutralizationComputer;
use prism_gpu::{
    BatchOutput, MegaFusedBatchGpu, MegaFusedConfig, PackedBatch, PolycentricImmunityGpu,
    StructureInput,
};
use prism_ve_model::{PRISMVEInput, PRISMVEPredictor, StructuralFeatures};
use temporal_immunity::TemporalImmunityComputer;
use vasil_data::{load_all_countries_enhanced, VasilEnhancedData};
use vasil_exact_metric::VasilMetricComputer;
use ve_optimizer::{AdaptiveVEOptimizer, VEAction, VEState};
use ve_swarm_integration::{PredictionInput, VasilEnhancedPredictor, VeSwarmPredictor};
...
fn compute_p_neut_series_75pk(
    ...
    for pk in pk_params {
        for week in 0..86 {
            ...
            let p_neut =
                compute_p_neut_single(epitope_escape, days_since_infection, pk, ic50_values);
            series.push(p_neut);
        }
    }
...
fn main() -> Result<()> {
    ...
    println!(
        "[CONFIG] Loaded parameters: neg={:.4}, min_freq={:.4}, IC50[0]={:.2}",
    ...
    let max_countries: usize = std::env::var("PRISM_COUNTRIES")
        ...
        .unwrap_or(12);
    println!(
        "\n[1/7] Loading VASIL countries (limit: {})...",
        max_countries
    );
    ...
    let total_lineages: usize = all_data
        .countries
        .iter()
        .map(|c| c.frequencies.lineages.len())
        .sum();
    let total_dates: usize = all_data
        .countries
        .iter()
    ...
    println!(
        "✅ Loaded {} countries, {} lineages, {} dates total",
    ...
    for country_data in &mut all_data.countries {
        ...
        if let Some(ve) = vasil_enhanced.get(&country_data.name) {
            ...
            eprintln!(
                "[INCIDENCE DIAG] {}: SOURCE=REAL_PHI, avg={:.2e}, pop={:.2e}",
            ...
            eprintln!(
                "[INCIDENCE DIAG] {}: SOURCE=FALLBACK, value={:.2e}",
                country_data.name, fallback_incidence
            );
        }
    }
    println!(
        "  ✅ Populated incidence data for {} countries with phi estimates",
        incidence_populated_count
    );
    ...
    for country_data in &all_data.countries {
        ...
        eprintln!(
            "{:15} {:10} (accuracy to be measured)",
            country_data.name, source
        );
    }
    eprintln!("═══════════════════════════════════════════════════════════\n");
    ...
    let top_lineages: Vec<String> = sorted_lineages
        .iter()
    ...
    println!(
        "  Found {} lineages with >1% peak frequency",
        top_lineages.len()
    );
    ...
    let temporal_computer = TemporalImmunityComputer::new().with_cache(top_lineages.clone());
    println!(
        "  ✅ Temporal immunity computer ready (cached {} variants)",
        top_lineages.len()
    );
    ...
    let (packed_batch, metadata) = build_mega_batch(
        &all_data,
        &structure_cache,
        train_cutoff,
        &optimized_params.ic50,
    )?;

    println!(
        "✅ Mega batch: {} structures ready for GPU",
        packed_batch.n_structures()
    );

    // Step 5: SINGLE GPU CALL processes EVERYTHING
    println!("\n[5/7] SINGLE GPU CALL - processing entire batch...");
    use std::io::Write;
    print!("  [DEBUG] Creating CUDA context...");
    std::io::stdout().flush()?;
    let context = cudarc::driver::CudaContext::new(0)?;
    println!(" OK");

    print!("  [DEBUG] Loading MegaFusedBatchGpu...");
    std::io::stdout().flush()?;
    let mut gpu = MegaFusedBatchGpu::new(context.clone(), Path::new("target/ptx"))?;
    println!(" OK");

    // POLYCENTRIC GPU ENABLED - Testing with 158-dim features
    print!("  [DEBUG] Loading PolycentricImmunityGpu...");
    std::io::stdout().flush()?;
    let mut polycentric =
        PolycentricImmunityGpu::new(context.clone(), Path::new("crates/prism-gpu/target/ptx"))?;
    println!(" OK");
    print!("  [DEBUG] Initializing epitope centers...");
    std::io::stdout().flush()?;
    {
        let n_samples = 100;
        let training_features: Vec<f32> = (0..n_samples * 136)
    ...
    print!("  [DEBUG] Calling detect_pockets_batch...");
    std::io::stdout().flush()?;
    eprintln!(
        "[DEBUG MAIN] About to call detect_pockets_batch with {} structures",
        packed_batch.n_structures()
    );
    ...
    eprintln!(
        "  - p_neut_75pk: {}",
        packed_batch.p_neut_time_series_75pk_packed.len()
    );
    eprintln!(
        "  - immunity_75: {}",
        packed_batch.current_immunity_levels_75_packed.len()
    );
    ...
    print!("  [DEBUG] Enhancing with polycentric features...");
    std::io::stdout().flush()?;
    ...
    println!(
        " OK ({:.2}s, features: 136 → 158 dim)",
        enhance_elapsed.as_secs_f32()
    );

    println!(
        "✅ GPU processed {} structures in {:.2}s (+ {:.2}s polycentric)",
        batch_output.structures.len(),
        batch_elapsed.as_secs_f32(),
        enhance_elapsed.as_secs_f32()
    );
    println!(
        "  Throughput: {:.0} structures/sec",
    ...
    println!(
        "  Training samples: {} (temporal holdout: < {})",
        train_data.len(),
        train_cutoff
    );
    println!(
        "  Testing samples: {} (temporal holdout: >= {})",
        test_data.len(),
        train_cutoff
    );

    // Analyze feature distributions
    let escape_range = train_data
        .iter()
        .map(|(s, _)| s.escape)
        .fold((f32::MAX, f32::MIN), |(min, max), v| {
            (min.min(v), max.max(v))
        });
    let ddg_bind_range = train_data
        .iter()
        .map(|(s, _)| s.ddg_binding)
        .fold((f32::MAX, f32::MIN), |(min, max), v| {
            (min.min(v), max.max(v))
        });
    let ddg_stab_range = train_data
        .iter()
        .map(|(s, _)| s.ddg_stability)
        .fold((f32::MAX, f32::MIN), |(min, max), v| {
            (min.min(v), max.max(v))
        });
    let expr_range = train_data
        .iter()
        .map(|(s, _)| s.expression)
        .fold((f32::MAX, f32::MIN), |(min, max), v| {
            (min.min(v), max.max(v))
        });
    let freq_range = train_data
        .iter()
        .map(|(s, _)| s.frequency)
        .fold((f32::MAX, f32::MIN), |(min, max), v| {
            (min.min(v), max.max(v))
    ...
    println!(
        "    escape:    {:.3} to {:.3}",
        escape_range.0, escape_range.1
    );
    println!(
        "    ddg_bind:  {:.3} to {:.3}",
        ddg_bind_range.0, ddg_bind_range.1
    );
    println!(
        "    ddg_stab:  {:.3} to {:.3}",
        ddg_stab_range.0, ddg_stab_range.1
    );
    ...
    let rise_escapes: Vec<f32> = train_data
        .iter()
        .filter(|(_, o)| *o == "RISE")
        .map(|(s, _)| s.escape)
        .collect();
    let fall_escapes: Vec<f32> = train_data
        .iter()
        .filter(|(_, o)| *o == "FALL")
        .map(|(s, _)| s.escape)
        .collect();
    ...
    println!(
        "  RISE escape mean: {:.3}, FALL escape mean: {:.3}",
        rise_mean, fall_mean
    );
    ...
    println!(
        "  Train RISE rate: {:.1}%",
        100.0 * train_rise as f32 / train_data.len() as f32
    );
    println!(
        "  Test RISE rate: {:.1}%",
        100.0 * test_rise as f32 / test_data.len() as f32
    );
    ...
    print!("  [VE-Swarm] Initializing 32 GPU agents...");
    std::io::stdout().flush()?;
    ...
    println!(
        "  [VE-Swarm] Training phase ({} samples)...",
        train_data.len()
    );
    ...
    for (idx, output) in batch_output.structures.iter().enumerate() {
        ...
        if let Some(structure) = structure_cache.get(&meta.lineage) {
            ...
            let freq_history = build_freq_history_for_sample(
                &all_data,
                &meta.country,
                &meta.lineage,
                meta.date_idx,
            ...
            match ve_swarm.predict_from_structure(
                ...
                Ok(prediction) => {
                    ...
                    if train_total % 500 == 0 {
                        ...
                        println!(
                            "    Progress: {}/{} samples, accuracy: {:.1}%, generation: {}",
                            train_total,
                            train_data.len(),
                            acc * 100.0,
                            ve_swarm.generation()
                ...
                Err(e) => {
                    ...
                    if train_predict_errors <= 5 {
                        println!(
                            "    [VE-Swarm ERROR {}] {}: {}",
                            train_predict_errors, meta.lineage, e
                        );
                    }
                }
            }
    ...
    let _ = (
        train_skipped_not_train,
        train_skipped_stable,
        train_structure_miss,
        train_predict_errors,
    );
    ...
    // Test phase
    println!(
        "  [VE-Swarm] Testing phase ({} samples)...",
        test_data.len()
    );
    ...
    println!("\n📊 VE-SWARM ANALYSIS:");
    println!(
        "  - GPU pipeline: WORKING ({:.0} structures/sec)",
        batch_output.structures.len() as f32 / batch_elapsed.as_secs_f32()
    );
    println!("  - Dendritic Reservoir: Preserved spatial info (125-dim × N_residues)");
    println!(
        "  - Swarm Intelligence: {} agents, {} generations",
        32,
        ve_swarm.generation()
    );
    ...
    if test_accuracy > 0.75 {
        println!(
            "\n🏆 EXCELLENT: {:.1}% - TARGET ACHIEVED!",
            test_accuracy * 100.0
        );
    } else if test_accuracy > 0.60 {
        println!(
            "\n✅ GOOD PROGRESS: {:.1}% - Swarm learning!",
            test_accuracy * 100.0
        );
    } else if test_accuracy > 0.50 {
        println!(
            "\n🔄 LEARNING: {:.1}% > 50% baseline",
            test_accuracy * 100.0
        );
    } else {
        println!(
            "\n⚠️  Below baseline: {:.1}% - need to tune swarm",
            test_accuracy * 100.0
        );
    }
    ...
    let train_refs: Vec<(VEState, &str)> = train_data
        .iter()
        .map(|(s, o)| (s.clone(), o.as_str()))
        .collect();
    optimizer.train_grid_search(&train_refs);
    let test_refs: Vec<(VEState, &str)> = test_data
        .iter()
        .map(|(s, o)| (s.clone(), o.as_str()))
        .collect();
    ...
    println!(
        "  VE-Swarm improvement: {:+.1}%",
        (test_accuracy - baseline_accuracy) * 100.0
    );
    ...
    if !vasil_enhanced.is_empty() {
        ...
        println!(
            "  Training on {} samples with VASIL features...",
            train_inputs.len()
        );
        ...
        println!(
            ...
            vasil_test_accuracy * 100.0,
            vasil_correct,
            test_inputs.len()
        );
        println!(
            "  Improvement over baseline: {:+.1}%",
            (vasil_test_accuracy - baseline_accuracy) * 100.0
        );
        println!(
            "  Improvement over VE-Swarm: {:+.1}%",
            (vasil_test_accuracy - test_accuracy) * 100.0
        );

        if vasil_test_accuracy > 0.85 {
            println!(
                "\n  TARGET ACHIEVED! {:.1}% > 85%",
                vasil_test_accuracy * 100.0
            );
        } else if vasil_test_accuracy > 0.75 {
            println!(
                "\n  GOOD PROGRESS: {:.1}% - approaching target",
                vasil_test_accuracy * 100.0
            );
        }
    } else {
    ...
    if !vasil_enhanced.is_empty() {
        ...
        println!(
            "\n  🎯 Fitting PRISM-VE weights on {} training samples...",
            prism_train_inputs.len()
        );
        ...
        println!(
            "  Test Accuracy: {:.1}% ({}/{})",
            prism_accuracy * 100.0,
            prism_correct,
            prism_test_inputs.len()
        );
        ...
        if prism_accuracy >= 0.94 {
            println!(
                "\n🏆 SUCCESS! BEAT VASIL: {:.1}% > 92.0%",
                prism_accuracy * 100.0
            );
        } else if prism_accuracy >= 0.90 {
            println!(
                "\n✅ STRONG: {:.1}% - approaching VASIL",
                prism_accuracy * 100.0
            );
        } else if prism_accuracy >= 0.75 {
            println!(
                "\n📈 PROGRESS: {:.1}% - significant improvement",
                prism_accuracy * 100.0
            );
        } else {
            println!(
                "\n⚠️  Below target: {:.1}% - need tuning",
                prism_accuracy * 100.0
            );
        }
    } else {
    ...
    if std::env::var("PRISM_ENABLE_PRISM4D").is_ok() {
        ...
        println!(
            "  Pre-computing structural ΔΔG matrix ({} variants)...",
            all_variants.len()
        );
        ...
        for meta in &metadata {
            ...
            let population =
                population_map.get(&meta.country).copied().unwrap_or(50.0) * 1_000_000.0;

            // PRISM-4D: Compute gamma using structural neutralization
            let gamma_y = prism_4d
                .compute_gamma(
                    &meta.country,
                    &meta.lineage,
                    meta.date,
            ...
            if prism_4d_total <= 5 {
                log::info!(
                    "  Sample {}: {} gamma={:.4}, predicted={}, actual={}",
                    prism_4d_total,
                    meta.lineage,
                    gamma_y,
        ...
        if prism_4d_accuracy >= 0.84 {
            ...
            println!(
                "\n⚠️  Below VE-Swarm: {:.1}% - formula needs tuning",
                prism_4d_accuracy * 100.0
            );
        }
    ...
    if std::env::var("PRISM_ENABLE_VASIL_METRIC").is_ok() {
        ...
        use vasil_exact_metric::{build_immunity_landscapes, VasilMetricComputer};
        ...
        println!(
            "  {:15} {:>12} {:>12} {:>15}",
            "Country", "Accuracy", "VASIL Target", "Delta"
        );
        println!("  {}", "-".repeat(70));
        ...
        if mean_accuracy >= 0.90 {
            println!(
                "\n🏆 SUCCESS! VASIL EXACT METRIC: {:.1}% ≈ 92%",
                mean_accuracy * 100.0
            );
            println!("   We match VASIL using their exact methodology!");
        } else if mean_accuracy >= 0.85 {
            println!(
                "\n✅ STRONG: {:.1}% - within 7 points of VASIL",
                mean_accuracy * 100.0
            );
            println!(
                "   Gap: {:.1} percentage points",
                (0.92 - mean_accuracy) * 100.0
            );
...
fn build_mega_batch(
    ...
    println!(
        "  Initialized immunity landscapes for {} countries",
        country_immunity.len()
    );
    ...
    println!(
        "  Built PK parameter grid: {} combinations",
        pk_params.len()
    );
    ...
    for country_data in &all_data.countries {
        ...
        let p_neut_series =
            compute_p_neut_series_75pk(&avg_epitope_escape, &pk_params, ic50_values);
        country_p_neut_75pk.insert(country_data.name.clone(), p_neut_series);
    }

    println!(
        "  Computed P_neut time series for {} countries (75 PK × 86 weeks)",
    ...
    println!(
        "  Pre-computed competition escape for {} country-date pairs",
        weighted_escape_cache.len()
    );
    ...
    for country_data in &all_data.countries {
        ...
        println!(
            "  {}: {} structures added",
            country_data.name, structures_added
        );
    }

    println!(
        "\nPacking {} structures into single batch...",
        all_inputs.len()
    );
    ...
    eprintln!(
        "[DEBUG] Computing immunity levels for {} structures...",
        all_metadata.len()
    );
    ...
    eprintln!(
        "[DEBUG] Immunity levels packed: {} values",
        immunity_levels_packed.len()
    );
    ...
    for (structure_idx, (structure, meta)) in all_inputs.iter().zip(all_metadata.iter()).enumerate()
    {
        ...
        let escapes: Vec<f32> = (0..10).map(|ep| meta.epitope_escape[ep]).collect();
    ...
    eprintln!(
        "[DMS GPU FIX] Epitope escape packed: {} values (expected: {})",
    ...
    println!(
        "     - P_neut: {} values ({} countries × 75 PK × 86 weeks)",
        packed.p_neut_time_series_75pk_packed.len(),
        all_data.countries.len()
    );
    println!(
        "     - Immunity levels: {} values ({} structures × 75 PK)",
        packed.current_immunity_levels_75_packed.len(),
        all_metadata.len()
    );
    println!(
        "     - PK params: {} values (75 × 4)",
...
fn report_per_country_results(
    ...
    for country_data in &all_data.countries {
        ...
        let country_test: Vec<_> = metadata
            .iter()
            .enumerate()
        ...
        let test_states: Vec<(VEState, &str)> = country_test
            .iter()
            .map(|(idx, _)| {
                ...
                let feature_dim = if output.combined_features.len() % 158 == 0 {
                    158
                } else {
                    136
                };
        ...
        let vasil_ref = vasil_targets
            .get(country_data.name.as_str())
            .copied()
            .unwrap_or(0.0);
