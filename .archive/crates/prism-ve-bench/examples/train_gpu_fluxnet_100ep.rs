...
impl GammaCache {
    ...
    fn load(cache_dir: &str) -> Option<Self> {
        ...
        if version != CACHE_VERSION {
            eprintln!(
                "[Cache] Version mismatch (found {}, need {}), will rebuild",
        ...
        if gamma_max.len() != n_samples
            || gamma_mean.len() != n_samples
            || actual_dirs.len() != n_samples
        {
...
fn main() -> Result<()> {
    ...
    let (gamma_min, gamma_max, gamma_mean, actual_dirs, n_samples) = if let Some(cache) =
        GammaCache::load(CACHE_DIR)
    {
        eprintln!(
            "  ✓ Loaded {} samples from cache (FAST PATH!)",
            cache.n_samples
        );
        (
            cache.gamma_min,
    ...
    eprintln!("════════════════════════════════════════════════════════════════════════");
    eprintln!(
        "  GPU-FUSED TRAINING ({} episodes × {} steps × {} configs)",
    ...
    eprintln!(
        "  Improvement:         {:+.2}%",
        (best_accuracy - BASELINE_ACCURACY) * 100.0
    );
    eprintln!();
    eprintln!("Training Stats:");
    eprintln!("  Total time:          {:.1}s", training_time.as_secs_f64());
    eprintln!(
        "  Time per episode:    {:.2}s",
        training_time.as_secs_f64() / N_EPISODES as f64
    );
    eprintln!();
    ...
    let toml_content = format!(
        r#"# GPU FluxNet Optimized Parameters (PATH B)
# Baseline: {:.2}%
# Achieved: {:.2}%
# Episodes: {}
...
fn extract_gamma_data_for_gpu(
    ...
    for country in countries {
        let major_lineages: Vec<&String> = country
            ...
            .iter()
            .filter(|lin| {
                let lineage_idx = country
                    .frequencies
                    .lineages
                    .iter()
                    .position(|l| l == *lin)
                    .unwrap_or(0);
                let max_freq = country
                    .frequencies
                    .frequencies
                    .iter()
        ...
        for lineage in major_lineages {
            ...
            for obs in observations {
                ...
                if let Ok(envelope) =
                    vasil_metric.compute_gamma_envelope_cached(&country.name, lineage, obs.date)
                {
