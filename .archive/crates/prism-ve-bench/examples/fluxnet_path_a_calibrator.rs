...
impl FluxNetCalibrator {
    ...
    fn update(
        ...
        let max_next_q = self.q_table[next_state_idx]
            .iter()
            .cloned()
...
fn evaluate_accuracy(
    ...
    let result = vasil_metric.compute_vasil_metric_exact(countries, eval_start, eval_end)?;
...
fn main() -> Result<()> {
    ...
    println!(
        "  Actions: {} (3 per param: increase/decrease/hold)",
        N_ACTIONS
    );
    ...
    for episode in 0..MAX_EPISODES {
        ...
        for step in 0..STEPS_PER_EPISODE {
            ...
            vasil_metric
                .set_path_a_mode(calibrator.params.epitope_weights, calibrator.params.sigma);
            vasil_metric.build_immunity_cache(
                dms_data,
                &all_data.countries,
                eval_start,
            ...
            // Next state
            let next_state =
                CalibrationState::new(new_accuracy, improvement > 0.0, steps_without_improvement);
        ...
        println!(
            "  Episode {} complete: total_reward={:.3}, best={:.2}%",
            episode + 1,
            episode_reward,
            calibrator.best_accuracy * 100.0
        );
    ...
    for (i, (name, weight)) in epitope_names
        .iter()
        .zip(calibrator.best_params.epitope_weights.iter())
        .enumerate()
    {
    ...
    println!(
        "  Improvement:        {:.2}%",
        (calibrator.best_accuracy - baseline_acc) * 100.0
    );
    println!("  PATH B Baseline:    79.0%");
    println!(
        "  vs PATH B:          {:+.2}%",
        (calibrator.best_accuracy - 0.79) * 100.0
    );
    println!();
    ...
    std::fs::write(
        "validation_results/fluxnet_calibrated_params.toml",
        &params_str,
    )?;
