...
fn main() -> Result<()> {
    ...
    println!(
        "  - Epitope weights: {} (contribution to P_neut)",
        N_EPITOPE_WEIGHTS
    );
    ...
    println!(
        "Q-Learning: α={}, γ={}, ε={}→{}",
        ALPHA, GAMMA, EPSILON_START, EPSILON_MIN
    );
    println!(
    ...
    println!(
        "  Improvement:        {:+.2}%",
        (optimizer.best_accuracy - baseline_acc) * 100.0
    );
    println!();
    ...
    let weight_names = [
        "A", "B", "C", "D1", "D2", "E12", "E3", "F1", "F2", "F3", "NTD",
    ];
    for (i, name) in weight_names.iter().enumerate() {
        println!(
            "  {}: {:.4}",
            name, optimizer.best_params.epitope_weights[i]
        );
    }
    println!();
    ...
    let params_str = format!(
        ...
        optimizer.best_params.epitope_weights[0],
        optimizer.best_params.epitope_weights[1],
        optimizer.best_params.epitope_weights[2],
        optimizer.best_params.epitope_weights[3],
        optimizer.best_params.epitope_weights[4],
        optimizer.best_params.epitope_weights[5],
        optimizer.best_params.epitope_weights[6],
        optimizer.best_params.epitope_weights[7],
        optimizer.best_params.epitope_weights[8],
        optimizer.best_params.epitope_weights[9],
        optimizer.best_params.epitope_weights[10],
        optimizer.best_params.rise_threshold,
        optimizer.best_params.fall_threshold,
    );

    std::fs::write(
        "validation_results/fluxnet_path_b_optimized.toml",
        &params_str,
    )?;
