...
"""
        ...
        # Tier 2 PRISM-TWIN Recommendation Card
        lining_residue_ids = [a.atom_label for a in am.anchors[:20]]
        residue_str = ", ".join(lining_residue_ids[:15])
        if len(lining_residue_ids) > 15:
            residue_str += f" (+{len(lining_residue_ids) - 15} more)"
        pdb_id_safe = brief.pdb_id.replace('"', '').replace("'", "")
        mailto_subject = (
            f"Tier 2 PRISM-TWIN request for {pdb_id_safe} "
            f"Site {brief.site_id}"
        ).replace(" ", "%20")

        html += f"""
<div style="margin: 2em 0; border: 2px solid #1565c0; border-radius: 8px; overflow: hidden;">
  <div style="background: #1565c0; color: white; padding: 12px 18px; font-size: 1.1em; font-weight: bold;">
    Recommended: PRISM-TWIN Tier 2 Analysis
  </div>
  <div style="padding: 18px; background: #e3f2fd;">
    <p style="margin-top: 0;">
      <strong>Focus residues (beacon):</strong> {residue_str}
    </p>
    <p><strong>Expected outputs:</strong></p>
    <ul>
      <li>Thermodynamic barrier (kcal/mol)</li>
      <li>TWIN classification (CONSENSUS_CRYPTIC / BARRIER_GATED / ALLOSTERIC_HUB / DYNAMIC_MODULATOR / INTERFACE_DEPENDENT / MUTATION_INDUCED / LIGAND_GATED)</li>
      <li>CCF allosteric coupling network</li>
      <li>Druggability with full MD-derived volume and hydration</li>
      <li>Per-residue TWIN features (48 dimensions)</li>
      <li>Mechanism of pocket opening</li>
    </ul>
    <p><strong>Estimated runtime:</strong> 5-15 minutes per target on GPU</p>
    <p style="margin-bottom: 12px;"><strong>Why this matters:</strong>
      Tier 2 resolves what Tier 1 cannot: the activation barrier, the opening
      mechanism, and whether the pocket is thermally accessible or requires
      perturbation. Sites classified as BARRIER_GATED by TWIN are invisible to
      all static methods &mdash; only coupled interferometric observation reveals them.
    </p>
    <a href="mailto:contact@prism4d.bio?subject={mailto_subject}"
       style="display: inline-block; background: #1565c0; color: white; padding: 10px 24px;
