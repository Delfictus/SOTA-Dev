...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Parameter sweep for coherence-first reclustering.")
    p.add_argument("root", type=Path, help="Single target dir or parent dir containing multiple target dirs")
    p.add_argument("--target", type=str, default=None, help="Optional single target name within root")
    p.add_argument("--out", type=Path, default=Path("coherence_sweep_results.json"), help="Output JSON")
    return p.parse_args()


def list_target_dirs(root: Path, target_name: Optional[str]) -> List[Path]:
    if target_name is not None:
        d = root / target_name
        if not d.is_dir():
            raise SystemExit(f"Target directory not found: {d}")
        return [d]
...
def main() -> None:
    args = parse_args()
    target_dirs = list_target_dirs(args.root, args.target)
    ...
    for cfg_idx, cfg in enumerate(configs, start=1):
        per_target = []
        print(
            f"[SWEEP] config {cfg_idx}/{len(configs)} "
            f"dist={cfg.max_pair_dist} cos={cfg.min_cosine} "
            f"lag={cfg.max_lag_diff} edge={cfg.min_edge_score}"
        )
        ...
        mean_dcc_gain = float(np.mean([r["dcc_gain"] for r in valid if r["dcc_gain"] is not None])) if any(r["dcc_gain"] is not None for r in valid) else None
        mean_pass_rate = float(np.mean([r["pass_rate"] for r in valid]))
        mean_cluster_size = float(np.mean([r["avg_cluster_size"] for r in valid]))
        total_passing = int(sum(r["n_passing_clusters"] for r in valid))
        ...
        all_results.append({
            ...
            "total_passing_clusters": total_passing,
            "per_target": valid,
        })
    ...
    all_results.sort(key=summary_rank, reverse=True)

    with open(args.out, "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2)

    print("\n=== Top sweep configs ===")
    for i, r in enumerate(all_results[:10], start=1):
        cfg = r["config"]
        print(
            f"{i:>2}. mean_dcc_gain={r['mean_dcc_gain']}  "
            f"pass_rate={r['mean_pass_rate']:.3f}  "
            f"avg_cluster_size={r['mean_cluster_size']:.2f}  "
            f"passing={r['total_passing_clusters']:>4}  "
