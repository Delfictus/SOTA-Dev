#!/usr/bin/env python3
"""
Apply nhs_spike_detect integration edits to fused_engine.rs

Replaces LIF-based spike detection (constant 2.0/step, structure-independent)
with displacement-based detection from nhs_spike_detect.cu kernel.

Usage:
    python3 apply_spike_detect_edits.py

Operates on: crates/prism-nhs/src/fused_engine.rs
Creates backup: crates/prism-nhs/src/fused_engine.rs.bak_lif
"""
...
def apply_edit(content, old, new, label):
    ...
    if count > 1:
        print(f"  ✗ {label}: pattern found {count} times (expected 1)")
        return content, False
...
def main():
    ...
    # ================================================================
    # EDIT 1: Add struct fields after d_aromatic_n_atoms
    # ================================================================
    old = (
    ...
    # This pattern may appear more than once if it's in both constructor and replica.
    # We want the FIRST occurrence (main constructor).
    count = content.count(old)
    if count == 0:
        print("  ✗ EDIT 3: struct literal pattern NOT FOUND")
        all_ok = False
    elif count >= 1:
        new = (
            '            d_aromatic_atom_indices,\n'
        ...
        content = content.replace(old, new, 1)
        print(f"  ✓ EDIT 3: struct literal ({count} occurrence(s), replaced first)")
    ...
    new = (
        ...
        '        // ================================================================\n'
        '        // Launch nhs_spike_detect: compares current positions to previous frame,\n'
        '        // emits spikes for aromatic atoms with displacement > 0.5A in isolated\n'
        '        // (pocket-like) environments. This replaces the LIF-based spike output.\n'
    ...
    content, ok = apply_edit(content, old, new, "EDIT 4: launch spike_detect after step")
    all_ok = all_ok and ok
    ...
    # This pattern may appear in both single-run and multi-stream paths.
    # Replace ALL occurrences — both should read from spike_detect.
    count = content.count(old)
    if count == 0:
        print("  ✗ EDIT 5: spike count read pattern NOT FOUND")
        all_ok = False
    else:
        content = content.replace(old, new)
        print(f"  ✓ EDIT 5: spike count read (replaced {count} occurrence(s))")
    ...
    content, ok = apply_edit(content, old, new, "EDIT 5c: read spike_detect_events for accumulation")
    all_ok = all_ok and ok

    # ================================================================
    # EDIT 6: Initialize prev_positions when topology is uploaded
    # ================================================================
    old = '        self.stream.memcpy_htod(&topology.positions, &mut self.d_positions)?;'
    count = content.count(old)
    if count == 0:
        print("  ✗ EDIT 6: position init pattern NOT FOUND")
        all_ok = False
    else:
        new = (
        ...
        # Replace first occurrence only (the main init path)
        content = content.replace(old, new, 1)
        print(f"  ✓ EDIT 6: initialize prev_positions ({count} occurrence(s), replaced first)")

    # ================================================================
    # WRITE
    # ================================================================
    if not all_ok:
        print("\n⚠ Some edits failed! Review output above.")
        print("Writing partial result anyway (backup preserved)...")

    with open(FILE, 'w') as f:
        f.write(content)

    new_len = len(content)
    print(f"\nDone. {original_len} -> {new_len} bytes (+{new_len - original_len})")
    print(f"Backup: {backup}")
