...
impl PdfReport {
    ...
    fn render_with_chromium(chromium: &Path, html_path: &Path, pdf_path: &Path) -> Result<()> {
        // Convert to file:// URL
    ...
    fn render_with_playwright(playwright: &Path, html_path: &Path, pdf_path: &Path) -> Result<()> {
        let script = format!(
            r#"
