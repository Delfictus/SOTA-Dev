//! ANM-Based Conformational Ensemble Generator
//!
//! Generates conformational ensembles by sampling Anisotropic Network Model (ANM)
//! normal modes. This matches the methodology used by PocketMiner and other MD-based
//! cryptic site detectors.
//!
//! # Physics Basis
//!
...
/// Configuration for ANM ensemble generation
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AnmEnsembleConfig {
    ...
    /// Typically 10-20 modes capture most of conformational variance
    pub n_modes: usize,

    /// Amplitude scaling factor (1.0 = physically realistic)
    /// Increase for larger conformational sampling
    pub amplitude_scale: f64,

    /// Maximum allowed displacement per residue (Ångstroms)
    /// Prevents unphysical distortions
...
impl Default for AnmEnsembleConfig {
    fn default() -> Self {
        Self {
            cutoff: 15.0,
            gamma: 1.0,
...
/// ANM Ensemble Generator
///
/// Generates conformational ensembles by sampling ANM normal modes.
/// This is a lightweight alternative to full MD simulation while
/// capturing the dominant conformational motions.
pub struct AnmEnsembleGenerator {
    config: AnmEnsembleConfig,
    rng: StdRng,
}

impl AnmEnsembleGenerator {
    /// Create a new generator with given configuration
    pub fn new(config: AnmEnsembleConfig) -> Self {
    ...
    pub fn default_config() -> Self {
        Self::new(AnmEnsembleConfig::default())
    }
    ...
    ///
    /// # Returns
    /// * `AnmEnsemble` containing original + generated conformations
    pub fn generate_ensemble(&mut self, ca_positions: &[[f32; 3]]) -> Result<AnmEnsemble> {
        ...
        log::info!(
            "Generating ANM ensemble: {} residues, {} conformations, {} modes",
            n,
        ...
        log::info!(
            "Ensemble generated: mean RMSD = {:.2}Å, max RMSD = {:.2}Å",
            mean_rmsd,
            max_rmsd
        );
    ...
    fn compute_anm_decomposition(&self, ca_positions: &[[f32; 3]]) -> Result<AnmDecomposition> {
        let n = ca_positions.len();
        let n3 = 3 * n;
        let cutoff_sq = self.config.cutoff * self.config.cutoff;

        log::debug!("Building {}×{} Hessian matrix", n3, n3);
        ...
        log::debug!("Computing eigendecomposition...");
    ...
    fn identify_dominant_modes(
        ...
        mode_variances
            .iter()
            .take(5)
            .map(|(idx, _)| decomp.valid_mode_indices[*idx])
            .collect()
    }
}
...
mod tests {
    ...
    #[test]
    fn test_anm_decomposition() {
        let coords = make_simple_helix(20);
        let config = AnmEnsembleConfig {
            n_conformations: 5,
            n_modes: 10,
            ..Default::default()
        };

        let mut gen = AnmEnsembleGenerator::new(config);
    ...
    fn test_ensemble_generation() {
        ...
        assert!(ensemble.mean_rmsd < 5.0); // Sanity check
    }
    ...
    fn test_max_displacement_constraint() {
        ...
        let config = AnmEnsembleConfig {
            ...
            max_displacement: 2.0, // But clamped to 2Å
            seed: Some(123),
            ..Default::default()
        };
        ...
        for conf in &ensemble.conformations {
            for (i, (orig, new)) in coords.iter().zip(conf.iter()).enumerate() {
                ...
                let disp = (dx * dx + dy * dy + dz * dz).sqrt();
                assert!(
                    disp <= 2.1, // Allow small tolerance
                    "Residue {} moved {:.2}Å, exceeds max 2.0Å",
                    i,
                    disp
                );
            }
