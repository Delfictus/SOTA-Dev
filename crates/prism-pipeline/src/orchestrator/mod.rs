...
use prism_gpu;
#[cfg(feature = "cuda")]
use prism_gpu::{GpuExecutionContext, GpuExecutionContextBuilder, MultiGpuContext, UltraKernelGpu};
#[cfg(feature = "lbs")]
use prism_lbs::{
...
impl PipelineOrchestrator {
    ...
    pub fn initialize_ultra_kernel(&mut self, graph: &Graph) -> Result<(), PrismError> {
        ...
        match UltraKernelGpu::new(
            ...
            Err(e) => {
                log::warn!(
                    "Ultra Kernel initialization failed: {}. Falling back to individual phases.",
                    e
                );
    ...
    pub fn run(&mut self, graph: &Graph) -> Result<ColoringSolution, PrismError> {
        ...
        #[cfg(feature = "cuda")]
        {
            if self.use_aatgs_async && self.gpu_exec_context.is_none() {
                if let Err(e) = self.initialize_aatgs() {
                    log::warn!(
                        "AATGS initialization failed: {}. Continuing with sync execution.",
                        e
                    );
                }
            }
        ...
        // =========================================================================
        #[cfg(feature = "cuda")]
        let mut geometry_sync: Option<GeometrySynchronizer> = if let Some(ref gpu_ctx_any) =
        ...
        if self.use_ultra_kernel {
            ...
            if let Err(e) = self.initialize_ultra_kernel(graph) {
                log::warn!(
                    "Ultra kernel init failed: {}. Falling back to phase-by-phase.",
                    e
                );
                self.use_ultra_kernel = false;
                ...
                match self.run_ultra_kernel(iterations) {
                    ...
                    Err(e) => {
                        log::warn!(
                            "Ultra kernel execution failed: {}. Falling back to phases.",
                            e
                        );
                        self.use_ultra_kernel = false;
                    }
 ...
 conflicts_before, conflicts_after);
                                // Revert would need solution backup - for now just warn
                                    ...
                                    log::info!(
                                        "WHCR-Phase2 completed: {} -> {} conflicts, geometry: {}",
                                        conflicts_before,
                                        conflicts_after,
                                        sync.summary()
                                    );
                    ...
                    if let Some(ref geom) = sync.geometry() {
                        ...
                        match self.invoke_whcr_phase3(graph, geom) {
                            Ok(()) => {
                                ...
                                if conflicts_after > conflicts_before * 3 {
                                    log::warn!(
                                        "WHCR-Phase3 oscillation detected: {} -> {} conflicts",
                                        conflicts_before,
                                        conflicts_after
                                    );
                                } else {
                                    log::info!(
                                        "WHCR-Phase3 completed: {} -> {} conflicts, geometry: {}",
                                        conflicts_before,
                                        conflicts_after,
                                        sync.summary()
                                    );
