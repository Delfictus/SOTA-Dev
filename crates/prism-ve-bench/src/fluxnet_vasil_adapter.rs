...
impl VasilParameters {
    ...
    pub fn from_toml_file(path: &str) -> Result<Self> {
        ...
        if let Some(thresholds) = toml.get("thresholds") {
            ...
            if let Some(v) = thresholds
                .get("min_peak_frequency")
                .and_then(|v| v.as_float())
            {
                params.min_peak_frequency = v as f32;
            }
            if let Some(v) = thresholds
                .get("confidence_margin")
                .and_then(|v| v.as_float())
            {
    ...
    pub fn load_optimized_or_default() -> Self {
        Self::from_toml_file("configs/optimized_params.toml").unwrap_or_else(|_| Self::default())
    }
}
...
impl VEFluxNetOptimizer {
    ...
    pub fn optimize_step(&mut self, state: &VEFluxNetState, explore: bool) -> VEFluxNetAction {
    ...
    pub fn record_result(
        ...
        if new_accuracy > self.best_accuracy {
            ...
            eprintln!(
                "[FluxNet VE] New best: {:.2}% with params: {:?}",
                new_accuracy * 100.0,
                self.params
            );
        }
    ...
    pub fn load(&mut self, path: &str) -> Result<()> {
        let content =
            std::fs::read_to_string(path).map_err(|e| anyhow!("Failed to read Q-table: {}", e))?;
        ...
        eprintln!(
            "[FluxNet VE] Loaded Q-table: {} episodes, best accuracy: {:.2}%",
            self.episodes,
