...
mod envelope_gpu_ffi {
    ...
    pub fn load_envelope_module(
        ...
        if !ptx_path.exists() {
            bail!(
                "PTX not found: {:?}. Run: nvcc -ptx gamma_envelope_reduction.cu",
                ptx_path
            );
        }
    ...
    pub fn gpu_compute_envelopes(
        ...
        let envelope_func = module
            .load_function("compute_gamma_envelopes_batch")
            .map_err(|e| anyhow!("Load envelope kernel: {}", e))?;

        // Allocate output buffers on GPU
        let mut d_gamma_min: CudaSlice<f64> = stream
            .alloc_zeros(n_samples)
            .map_err(|e| anyhow!("Alloc gamma_min: {}", e))?;
        let mut d_gamma_max: CudaSlice<f64> = stream
            .alloc_zeros(n_samples)
            .map_err(|e| anyhow!("Alloc gamma_max: {}", e))?;
        let mut d_gamma_mean: CudaSlice<f64> = stream
            .alloc_zeros(n_samples)
        ...
        eprintln!(
            "[GPU Envelope] Launching kernel: {} samples, {} blocks (per-PK weighted_avg)",
            n_samples, grid_size
        );
        ...
        stream
            .synchronize()
            .map_err(|e| anyhow!("GPU sync failed: {}", e))?;

        // Download ONLY reduced results (min, max, mean) - NOT all 75 values
        let gamma_min: Vec<f64> = stream
            .clone_dtoh(&d_gamma_min)
            .map_err(|e| anyhow!("Download gamma_min: {}", e))?;
        let gamma_max: Vec<f64> = stream
            .clone_dtoh(&d_gamma_max)
            .map_err(|e| anyhow!("Download gamma_max: {}", e))?;
        let gamma_mean: Vec<f64> = stream
            .clone_dtoh(&d_gamma_mean)
            .map_err(|e| anyhow!("Download gamma_mean: {}", e))?;

        eprintln!(
            "[GPU Envelope] ✓ Downloaded envelope results ({} samples)",
            n_samples
        );
    ...
    pub fn gpu_compute_weighted_avg_susceptibility(
        ...
        let weighted_avg_func = module
            .load_function("compute_weighted_avg_susceptibility")
            .map_err(|e| anyhow!("Load weighted_avg kernel: {}", e))?;

        // Allocate output buffer on GPU - FIXED: n_samples * 75 for per-PK output!
        let mut d_weighted_avg_75pk: CudaSlice<f64> = stream
            .alloc_zeros(n_total_outputs)
        ...
        eprintln!(
            "[GPU Weighted Avg] Launching kernel: {} samples × 75 PKs = {} threads, {} blocks",
        ...
        let weighted_avg_vec: Vec<f64> = stream
            .clone_dtoh(&d_weighted_avg_75pk)
            .map_err(|e| anyhow!("Download weighted_avg_75pk: {}", e))?;

        eprintln!(
            "[GPU Weighted Avg] ✓ Downloaded results ({} samples × 75 = {} values)",
...
fn is_ntd_supersite(site: u32) -> bool {
    (14..=20).contains(&site) || (141..=158).contains(&site) || (245..=264).contains(&site)
}
...
fn count_ntd_mutation_differences(mutations_x: &[String], mutations_y: &[String]) -> u32 {
    let ntd_x: HashSet<u32> = mutations_x
        .iter()
        .filter_map(|m| parse_mutation_site(m))
        .filter(|&site| is_ntd_supersite(site))
        .collect();

    let ntd_y: HashSet<u32> = mutations_y
        .iter()
        .filter_map(|m| parse_mutation_site(m))
        .filter(|&site| is_ntd_supersite(site))
        .collect();
...
pub const THALF_VALUES: [f32; 15] = [
    25.0, 28.14, 31.29, 34.43, 37.57, 40.71, 43.86, 47.0, 50.14, 53.29, 56.43, 59.57, 62.71, 65.86,
    69.0,
];
...
impl FoldResistanceMatrix {
    ...
    pub fn from_dms_data_with_ic50(
        ...
        for lineage_x in lineages.iter() {
            for lineage_y in lineages.iter() {
                for (epitope_idx, _epitope) in EPITOPE_CLASSES.iter().enumerate() {
                    let fr =
                        matrix.compute_fold_resistance(dms_data, lineage_x, lineage_y, epitope_idx);
                    matrix
                        .fr
                        .insert((lineage_x.clone(), lineage_y.clone(), epitope_idx), fr);
                }
            }
        }
    ...
    fn compute_fold_resistance(
        ...
        let escape_x = dms_data
            .get_epitope_escape(lineage_x, epitope_idx)
            .unwrap_or(0.0);
        let escape_y = dms_data
            .get_epitope_escape(lineage_y, epitope_idx)
            .unwrap_or(0.0);
    ...
    pub fn get_fr(&self, lineage_x: &str, lineage_y: &str, epitope_idx: usize) -> f32 {
        self.fr
            .get(&(lineage_x.to_string(), lineage_y.to_string(), epitope_idx))
            .copied()
            .unwrap_or(1.0)
    }
}
...
impl ImmunityCache {
    ...
    pub fn build_for_landscape(
        ...
        eprintln!(
            "[ImmunityCache] Building for {} variants × {} days...",
            n_variants, n_days
        );
        ...
        for (y_idx, lineage_y) in landscape.lineages.iter().enumerate() {
            if y_idx % 20 == 0 {
                eprintln!(
                    "[ImmunityCache] Processing variant {}/{}",
                    y_idx, n_variants
                );
            }
    ...
    #[inline]
    pub fn get_gamma_envelope(
        &self,
        orig_variant_idx: usize,
        date: NaiveDate,
    ) -> Option<(f64, f64, f64)> {
        ...
        if sig_idx < self.gamma_envelopes.len() && day_offset < self.gamma_envelopes[sig_idx].len()
        {
    ...
    pub fn get_susceptible(&self, orig_variant_idx: usize, date: NaiveDate) -> f64 {
        ...
        if let Some((_, _, gamma_mean)) = self.get_gamma_envelope(orig_variant_idx, date) {
            ...
            (self.population * (1.0 + gamma_mean) / 2.0)
                .max(0.0)
                .min(self.population)
        } else {
    ...
    fn compute_p_neut_with_ntd(
        ...
        // ════════════════════════════════════════════════════════════════════════
        let mutations_x = lineage_mutations
            .get(lineage_x)
            .map(|v| v.as_slice())
            .unwrap_or(&[]);
        let mutations_y = lineage_mutations
            .get(lineage_y)
            .map(|v| v.as_slice())
            .unwrap_or(&[]);
    ...
    pub fn build_for_landscape_gpu(
        ...
        let thalf_values: [f32; 15] = [
            25.0, 28.14, 31.29, 34.43, 37.57, 40.71, 43.86, 47.0, 50.14, 53.29, 56.43, 59.57,
            62.71, 65.86, 69.0,
        ];

        // Filter to significant variants (≥1% peak frequency at any point)
        let significant_indices: Vec<usize> = landscape
            .lineages
            .iter()
            .enumerate()
            .filter(|(idx, _)| {
        ...
        eprintln!(
            "  {} significant variants (of {} total)",
            n_variants,
            landscape.lineages.len()
        );
        eprintln!("  {} eval days × 75 PK combinations", n_eval_days);
        eprintln!(
            "  {} days history, eval offset {}",
            max_history_days, eval_start_offset
        );
        ...
        let ptx_src =
            std::fs::read_to_string(ptx_path).map_err(|e| anyhow!("Failed to read PTX: {}", e))?;

        let module = context
            .load_module(Ptx::from_src(ptx_src))
            .map_err(|e| anyhow!("Failed to load PTX module: {}", e))?;

        let compute_immunity_func = module
            .load_function("compute_immunity_onthefly")
            .map_err(|e| anyhow!("Failed to load compute_immunity_onthefly: {}", e))?;

        // ═══════════════════════════════════════════════════════════════════
        // STEP 5: Allocate GPU buffers
        // ═══════════════════════════════════════════════════════════════════
        let mut d_epitope_escape: CudaSlice<f32> = stream
            .alloc_zeros(n_variants * N_EPITOPES)
            .map_err(|e| anyhow!("GPU alloc epitope_escape: {}", e))?;
        let mut d_frequencies: CudaSlice<f32> =
            stream
                .alloc_zeros(n_variants * max_history_days)
                .map_err(|e| anyhow!("GPU alloc frequencies: {}", e))?;
        let mut d_incidence: CudaSlice<f64> = stream
            .alloc_zeros(max_history_days)
        ...
        let mut d_immunity: CudaSlice<f64> = stream
            .alloc_zeros(n_variants * n_eval_days * 75)
        ...
        // ═══════════════════════════════════════════════════════════════════
        stream
            .memcpy_htod(&epitope_escape, &mut d_epitope_escape)
            .map_err(|e| anyhow!("Upload epitope_escape: {}", e))?;
        stream
            .memcpy_htod(&frequencies, &mut d_frequencies)
            .map_err(|e| anyhow!("Upload frequencies: {}", e))?;
        stream
            .memcpy_htod(&incidence, &mut d_incidence)
        ...
        let immunity_all: Vec<f64> = stream
            .clone_dtoh(&d_immunity)
            .map_err(|e| anyhow!("Download immunity: {}", e))?;

        // Reshape into [variant][day][pk] structure
        let mut immunity_matrix_75pk: Vec<Vec<[f64; 75]>> =
            vec![vec![[0.0; 75]; n_eval_days]; n_variants];

        for y_idx in 0..n_variants {
            for t_idx in 0..n_eval_days {
                for pk_idx in 0..75 {
                    let offset =
                        (pk_idx * n_variants * n_eval_days) + (y_idx * n_eval_days) + t_idx;
        ...
        eprintln!(
            "[Phase 1 GPU] ✓ Reshaped to {} variants × {} days × 75 PK",
        ...
        let mut d_immunity_75pk_flat: CudaSlice<f64> = stream
            .alloc_zeros(n_samples * 75)
        ...
        stream
            .memcpy_htod(&immunity_flat, &mut d_immunity_75pk_flat)
        ...
        eprintln!(
            "[Phase 1 GPU] ✓ Weighted avg computed ({} samples × 75 PKs)",
            n_samples
        );
        ...
        stream
            .memcpy_htod(&weighted_avg_vec, &mut d_weighted_avg_75pk)
        ...
        let mut gamma_envelopes: Vec<Vec<(f64, f64, f64)>> =
            vec![vec![(0.0, 0.0, 0.0); n_eval_days]; n_variants];
        for y_idx in 0..n_variants {
            for t_idx in 0..n_eval_days {
        ...
        eprintln!(
            "[Phase 1 GPU] ✓ Gamma envelopes computed on GPU ({} samples)",
            n_samples
        );
        ...
        eprintln!(
            "[ImmunityCache GPU] ✓ Built in {:.2}s ({:.1} PK/sec)",
            elapsed.as_secs_f64(),
        ...
        eprintln!(
            "  - immunity_matrix_75pk: {} variants × {} days × 75 PK",
            n_variants, n_eval_days
        );
    ...
    pub fn build_for_landscape_gpu_path_a(
        ...
        let mut d_epitope_escape = stream
            .alloc_zeros(n_variants * N_EPITOPES)
        ...
        let mut d_epitope_weights = stream
            .alloc_zeros(N_EPITOPES)
            .map_err(|e| anyhow!("Alloc weights: {}", e))?;
        stream
            .memcpy_htod(epitope_weights, &mut d_epitope_weights)
        ...
        // ═══════════════════════════════════════════════════════════════════
        eprintln!(
            "[PATH A] Computing epitope-based P_neut matrix ({} × {})...",
        ...
        let compute_p_neut_func = module
            .load_function("compute_epitope_p_neut")
            .map_err(|e| anyhow!("Load function: {}", e))?;

        // Allocate P_neut matrix
        let mut d_p_neut_matrix = stream
            .alloc_zeros(n_variants * n_variants)
        ...
        // Upload incidence
        let mut d_incidence = stream
            .alloc_zeros(max_history_days)
            .map_err(|e| anyhow!("Alloc incidence: {}", e))?;
        stream
            .memcpy_htod(&landscape.daily_incidence, &mut d_incidence)
            .map_err(|e| anyhow!("Upload incidence: {}", e))?;

        // Allocate immunity output [n_variants × n_eval_days]
        let mut d_immunity: CudaSlice<f64> = stream
            .alloc_zeros(n_variants * n_eval_days)
        ...
        let compute_immunity_func = module
            .load_function("compute_immunity_from_epitope_p_neut")
        ...
        eprintln!(
            "[PATH A GPU] Launching immunity kernel: grid=({}, {}), block={}, smem={}B",
        ...
        stream
            .synchronize()
            .map_err(|e| anyhow!("Sync immunity: {}", e))?;

        // Download immunity matrix from GPU
        let immunity_matrix: Vec<f64> = stream
            .clone_dtoh(&d_immunity)
            .map_err(|e| anyhow!("Download immunity: {}", e))?;

        eprintln!(
            "[PATH A GPU] ✓ Immunity computed on GPU ({}× faster than CPU)",
        ...
        for y_idx in 0..n_variants {
            for t_idx in 0..n_eval_days {
                ...
                if t_abs < max_history_days {
                    for x_idx in 0..n_variants {
                        ...
                        if freq >= 0.001 {
                            ...
                            let susceptibility_x = (landscape.population
                                - immunity_x.min(landscape.population))
                            .max(0.0);
                ...
                let susceptibility_y =
                    (landscape.population - immunity_y.min(landscape.population)).max(0.0);
        ...
        let elapsed = start_time.elapsed();
        eprintln!(
            "[ImmunityCache GPU PATH A] ✓ Built in {:.2}s",
            elapsed.as_secs_f64()
        );
        ...
        eprintln!(
            "  - immunity_matrix: {} variants × {} days (single profile)",
            n_variants, n_eval_days
        );
        eprintln!(
            "  - gamma_envelopes: {} samples (deterministic)",
            n_variants * n_eval_days
        );
        eprintln!("  - Date range: {:?} to {:?}", min_date, max_date);
...
impl VariantIndex {
    fn build(lineages: &[String]) -> Self {
        let name_to_idx: HashMap<String, u32> = lineages
            .iter()
            .enumerate()
...
impl VasilGammaComputer {
    ...
    pub fn build_immunity_cache(
        ...
        eprintln!(
            "[VasilGamma] Building immunity cache for {} countries...",
            self.landscapes.len()
        );
        ...
        eprintln!(
            "[NTD] Loaded mutations for {} lineages",
            all_mutations.len()
        );
    ...
    pub fn compute_gamma_cached(
        ...
        let lineage_y_idx = landscape
            .get_lineage_idx(lineage_y)
        ...
        static SA_GAMMA_DEBUG: std::sync::atomic::AtomicUsize =
    ...
    pub fn compute_gamma_envelope_cached(
        ...
        let (min, max, mean) = cache
            .get_gamma_envelope(lineage_y_idx, date)
    ...
    pub fn compute_gamma_envelope(
        ...
        for pk in &self.pk_grid {
            let gamma =
                self.compute_gamma_single_pk(landscape, lineage_y, lineage_y_idx, date, pk)?;
    ...
    fn compute_gamma_single_pk(
        ...
        // E[Sy(t)] = Pop - E[Immuney(t)]
        let e_immune_y =
            self.compute_expected_immune(landscape, lineage_y, lineage_y_idx, date, pk);
        ...
        for (x_idx, lineage_x) in landscape.lineages.iter().enumerate() {
            ...
            let e_immune_x = self.compute_expected_immune(landscape, lineage_x, x_idx, date, pk);
    ...
    fn compute_expected_immune(
        ...
        while integration_date < date {
            ...
            for (x_idx, lineage_x) in landscape.lineages.iter().enumerate() {
                ...
                let p_neut = self.compute_p_neut(lineage_x, lineage_y, t_minus_s, pk);
    ...
    fn compute_p_neut(
        ...
        for (epitope_idx, _epitope) in EPITOPE_CLASSES.iter().enumerate() {
            ...
            let fr = self
                .fold_resistance
                .get_fr(lineage_x, lineage_y, epitope_idx);
...
impl VasilMetricComputer {
    ...
    pub fn build_immunity_cache(
        ...
        self.gamma_computer.build_immunity_cache(
            dms_data,
            all_countries_data,
            eval_start,
            eval_end,
    ...
    pub fn compute_gamma_cached(
        ...
        self.gamma_computer
            .compute_gamma_cached(country, lineage, date)
    }
    ...
    pub fn compute_gamma_envelope_cached(
        ...
        self.gamma_computer
            .compute_gamma_envelope_cached(country, lineage, date)
    }
    ...
    pub fn partition_frequency_curve(
        ...
        let lineage_idx = match country_data
            .frequencies
            .lineages
            .iter()
        ...
        for (date_idx, date) in country_data.frequencies.dates.iter().enumerate() {
            ...
            let freq_tomorrow = country_data
                .frequencies
                .frequencies
                .get(date_idx + 1)
                .and_then(|row| row.get(lineage_idx))
    ...
    pub fn compute_with_gpu_kernel(
        ...
        let kernel_func = module
            .load_function("mega_fused_vasil_fluxnet")
        ...
        for country in all_countries {
            ...
            let major_lineages: Vec<(usize, &String)> = country
                .frequencies
                .lineages
                .iter()
                .enumerate()
            ...
            let eval_start_offset =
                (evaluation_start - country.frequencies.dates[0]).num_days() as i32;

            eprintln!(
                "[GPU-NATIVE] {}: {} major variants (of {} total)",
                country.name,
                n_variants,
            ...
            for (new_idx, (orig_idx, lineage)) in major_lineages.iter().enumerate() {
                for epitope_idx in 0..10 {
                    if let Some(escape_val) =
                        country.dms_data.get_epitope_escape(lineage, epitope_idx)
                    {
            ...
            let mut d_frequencies: CudaSlice<f32> =
                stream.alloc_zeros(n_variants * max_history_days)?;
            let mut d_incidence: CudaSlice<f64> = stream.alloc_zeros(max_history_days)?;
            let mut d_actual_directions: CudaSlice<i8> =
                stream.alloc_zeros(n_variants * n_eval_days)?;
            let mut d_freq_changes: CudaSlice<f32> =
                stream.alloc_zeros(n_variants * n_eval_days)?;
        ...
        eprintln!(
            "[GPU-NATIVE] Total GPU time: {:.2}s for {} countries",
            gpu_total_time.as_secs_f64(),
            all_countries.len()
        );
    ...
    pub fn compute_with_gpu_kernel_batched(
        ...
        eprintln!(
            "[ES-BATCH] Evaluating {} parameter sets...",
            param_sets.len()
        );
        let batch_start = std::time::Instant::now();

        for (i, &(ref ic50, ref power, rise_bias, fall_bias, gamma_threshold)) in
            param_sets.iter().enumerate()
        {
            // Temporarily update IC50 for this evaluation
            let old_ic50 = *self.gamma_computer.get_ic50();
            self.gamma_computer.set_ic50([
                ic50[0], ic50[1], ic50[2], ic50[3], ic50[4], ic50[5], ic50[6], ic50[7], ic50[8],
                ic50[9],
            ]);
        ...
        eprintln!(
            "[ES-BATCH] Complete: {} evals in {:.1}s ({:.2}s/eval)",
            param_sets.len(),
    ...
    pub fn compute_vasil_metric_exact(
        ...
        if strict_mode {
            ...
            if !has_75pk_envelope {
                bail!(
                    "STRICT MODE FAILURE: 75-PK envelope not implemented\n\
        ...
        for country in all_countries {
            ...
            let major_lineages: Vec<&String> = country
                .frequencies
                .lineages
                .iter()
            ...
            for lineage in &major_lineages {
                ...
                for obs in &observations {
                    ...
                    static ENVELOPE_DEBUG: std::sync::atomic::AtomicUsize =
                        std::sync::atomic::AtomicUsize::new(0);
                    if lineage_total < 3 {
                        let count =
                            ENVELOPE_DEBUG.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                        if count < 10 {
                            eprintln!(
                                "\n[ENVELOPE DIAGNOSTIC] {}/{} @ {:?}",
                            ...
                            eprintln!(
                                "  Envelope: min={:.6}, max={:.6}, mean={:.6}",
                                envelope.min, envelope.max, envelope.mean
                            );
                            eprintln!("  Is decided: {}", envelope.is_decided);
                            eprintln!("  Direction: {:?}", envelope.direction);
                            eprintln!("  Actual: {:?}", actual_direction);
                            eprintln!(
                                "  Frequency: {:.4}, Change: {:.4}",
                                obs.frequency, obs.frequency_change
                            );
                        }
                    }
                    ...
                    let envelope_decision =
                        EnvelopeDecision::from_envelope(envelope.min as f64, envelope.max as f64);
                    ...
                    let predicted_direction = envelope_decision
                        .to_day_direction()
        ...
        let mean_accuracy = if !per_lineage_country_accuracy.is_empty() {
            per_lineage_country_accuracy
                ...
                .map(|(_, _, acc, _)| acc)
                .sum::<f32>()
                / per_lineage_country_accuracy.len() as f32
        } else {
            0.0
        };
        ...
        // ════════════════════════════════════════════════════════════════════════════
        {
            ...
            let sum_acc: f32 = per_lineage_country_accuracy
                .iter()
                .map(|(_, _, acc, _)| *acc)
                .sum();
            ...
            eprintln!(
                "  Stored mean_accuracy:           {:.4} ({:.2}%)",
                mean_accuracy,
                mean_accuracy * 100.0
            );
            eprintln!(
                "  Match check:                    {}",
                (computed_mean - mean_accuracy).abs() < 0.0001
            );
            ...
            let above_90 = per_lineage_country_accuracy
                .iter()
                .filter(|(_, _, a, _)| *a > 0.90)
                .count();
            let above_80 = per_lineage_country_accuracy
                .iter()
                .filter(|(_, _, a, _)| *a > 0.80 && *a <= 0.90)
                .count();
            let above_70 = per_lineage_country_accuracy
                .iter()
                .filter(|(_, _, a, _)| *a > 0.70 && *a <= 0.80)
                .count();
            let above_60 = per_lineage_country_accuracy
                .iter()
                .filter(|(_, _, a, _)| *a > 0.60 && *a <= 0.70)
                .count();
            let above_50 = per_lineage_country_accuracy
                .iter()
                .filter(|(_, _, a, _)| *a > 0.50 && *a <= 0.60)
                .count();
            let at_or_below_50 = per_lineage_country_accuracy
                .iter()
                .filter(|(_, _, a, _)| *a <= 0.50)
                .count();
            ...
            eprintln!(
                "  >90%:   {:4} lineages ({:.1}%)",
                above_90,
                above_90 as f32 / total_pairs as f32 * 100.0
            );
            eprintln!(
                "  80-90%: {:4} lineages ({:.1}%)",
                above_80,
                above_80 as f32 / total_pairs as f32 * 100.0
            );
            eprintln!(
                "  70-80%: {:4} lineages ({:.1}%)",
                above_70,
                above_70 as f32 / total_pairs as f32 * 100.0
            );
            eprintln!(
                "  60-70%: {:4} lineages ({:.1}%)",
                above_60,
                above_60 as f32 / total_pairs as f32 * 100.0
            );
            eprintln!(
                "  50-60%: {:4} lineages ({:.1}%)",
                above_50,
                above_50 as f32 / total_pairs as f32 * 100.0
            );
            eprintln!(
                "  ≤50%:   {:4} lineages ({:.1}%)",
                at_or_below_50,
                at_or_below_50 as f32 / total_pairs as f32 * 100.0
            );

            // Prediction count analysis
            let total_preds: usize = per_lineage_country_accuracy
                .iter()
                .map(|(_, _, _, n)| *n)
                .sum();
            let avg_preds = total_preds as f32 / total_pairs as f32;
            let min_preds = per_lineage_country_accuracy
                .iter()
                .map(|(_, _, _, n)| *n)
                .min()
                .unwrap_or(0);
            let max_preds = per_lineage_country_accuracy
                .iter()
                .map(|(_, _, _, n)| *n)
                .max()
                .unwrap_or(0);
...
impl std::fmt::Display for VasilMetricResult {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "Excluded (negligible): {}",
            self.total_excluded_negligible
        )?;
        writeln!(
            f,
            "Excluded (undecided):  {}",
            self.total_excluded_undecided
        )?;
...
pub fn veswarm_to_gamma_envelope(fitness_score: f32, confidence: f32) -> GammaEnvelope {
...
pub fn build_immunity_landscapes(
    ...
    for country in all_countries {
        ...
        let vaccination_fraction = country.vaccination_data.clone().unwrap_or_else(|| {
...
mod tests {
    ...
    fn test_pk_params() {
        ...
        assert!(
            c_at_2tmax < c_at_tmax,
            "c(2*tmax) should be less than c(tmax)"
        );
