...
use prism_ve_bench::vasil_exact_metric::{
    build_immunity_landscapes, VasilMetricComputer, CALIBRATED_IC50,
};
                    ...
                    match action_type {
                        0 => {
                            self.negligible_threshold =
                                (self.negligible_threshold + THRESHOLD_STEP).min(0.15)
                        }
                        1 => {
                            self.negligible_threshold =
                                (self.negligible_threshold - THRESHOLD_STEP).max(0.01)
                        }
                        _ => {}
                    }
                }
                    ...
                    match action_type {
                        0 => {
                            self.min_peak_frequency =
                                (self.min_peak_frequency + THRESHOLD_STEP).min(0.05)
                        }
                        1 => {
                            self.min_peak_frequency =
                                (self.min_peak_frequency - THRESHOLD_STEP).max(0.005)
                        }
                        _ => {}
                    }
                }
...
impl StructuredParams {
    fn to_linear(&self) -> ([f32; 10], [f32; 10], f32, f32, f32) {
        ...
        (
            ic50,
            power,
            self.rise_bias as f32,
            self.fall_bias as f32,
            self.gamma_threshold as f32,
        )
    }

    fn apply_structured_noise(
        &self,
        noise_log_ic50: &[f64],
        noise_power: &[f64],
        noise_bias: &[f64],
...
impl PrismES {
    ...
    fn generate_population(&self) -> Vec<(StructuredParams, Vec<f64>)> {
        ...
        (0..N_POPULATION)
            .map(|_| {
                ...
                let noise_log_ic50: Vec<f64> = (0..11)
                    .map(|_| {
                        let u1: f64 = rng.gen();
                        let u2: f64 = rng.gen();
                ...
                let perturbed = self.base_params.apply_structured_noise(
                    &noise_log_ic50,
                    &noise_power,
                    &noise_bias,
                );
    ...
    fn update(&mut self, population: &[(StructuredParams, Vec<f64>)], fitnesses: &[f64]) {
        ...
        let squared_diffs: Vec<f64> = fitnesses
            .iter()
        ...
        // Clamp to valid ranges
        for i in 0..11 {
            self.base_params.log_ic50[i] =
                self.base_params.log_ic50[i].clamp((0.1f64).ln(), (5.0f64).ln());
        ...
        // Track best
        if let Some((idx, &best_fit)) = fitnesses
            .iter()
            .enumerate()
...
fn main() -> Result<()> {
    ...
    eprintln!(
        "[Config] Sigma: IC50={}, power={}, bias={}",
        SIGMA_IC50_INIT, SIGMA_POWER_INIT, SIGMA_BIAS_INIT
    );
    eprintln!();
    ...
    eprintln!(
        "     Total predictions: {}",
        baseline_result.total_predictions
    );
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    if false {
        // DISABLED - 0.00 improvement over 49 combinations
        ...
        for &rise_bias in &bias_values {
            for &fall_bias in &bias_values {
                ...
                if acc > best_acc {
                    ...
                    eprintln!(
                        "[BIAS SWEEP] NEW BEST: {:.2}% (rise={:.2}, fall={:.2}) [{}/{}]",
                        acc * 100.0,
                        rise_bias,
                        fall_bias,
                        combo_count,
                ...
                if combo_count % 7 == 0 {
                    eprintln!(
                        "[BIAS SWEEP] Progress: {}/{} combos ({:.1}s elapsed)",
                        combo_count,
                        total_combos,
        ...
        eprintln!("════════════════════════════════════════════════════════════════════════");
        eprintln!(
            "  Baseline (rise=0.0, fall=0.0): {:.2}%",
            baseline_acc * 100.0
        );
        eprintln!(
            "  Best (rise={:.2}, fall={:.2}): {:.2}%",
            best_rise,
            best_fall,
            best_acc * 100.0
        );
        eprintln!(
            ...
            (best_acc - baseline_acc) * 100.0
        );
        eprintln!(
            "  Sweep time: {:.1}s ({} combos)",
    ...
    for generation in 0..generations {
        ...
        eprintln!(
            ...
            generation + 1,
            generations,
            es_optimizer.best_fitness * 100.0
        );
        ...
        let param_sets: Vec<([f32; 10], [f32; 10], f32, f32, f32)> = population
            .iter()
    ...
    eprintln!(
        "  Improvement:   {:+.2}%",
        (es_optimizer.best_fitness - baseline_acc) * 100.0
    );
    eprintln!();
    ...
    eprintln!(
        "  Time per gen:  {:.1}s",
        training_time.as_secs_f64() / generations as f64
    );
    eprintln!();

    // Print optimized params (convert from log space)
    let (best_ic50, best_power, best_rise, best_fall, best_threshold) =
        es_optimizer.best_params.to_linear();
