...
impl GisaidFrequencies {
    ...
    pub fn load_from_vasil(vasil_data_dir: &Path, country: &str) -> Result<Self> {
        ...
        let lineages: Vec<String> = headers
            .iter()
            .skip(1)
...
impl DmsEscapeData {
    ...
    pub fn load_from_vasil(vasil_dir: &Path, country: &str) -> Result<Self> {
        ...
        let dms_paths = [
            vasil_dir
                .join("ByCountry")
                .join(country_normalized)
                .join("results")
                .join("epitope_data")
                .join("dms_per_ab_per_site.csv"),
            vasil_dir
                .join("ByCountry")
                .join(country)
                .join("results")
        ...
        // Diagnostic output
        eprintln!(
            "[DMS] Loaded: {} antibodies, {} lineages, real_data={}",
            result.n_antibodies,
    ...
    fn load_dms_file(&mut self, path: &Path) -> Result<()> {
        ...
        let header = lines.next().ok_or_else(|| {
            std::io::Error::new(std::io::ErrorKind::InvalidData, "Empty DMS file")
        })??;
    ...
    fn load_mutations_file(&mut self, path: &Path) -> Result<()> {
        ...
        eprintln!(
            "[DMS] Loaded mutations for {} lineages",
            self.lineage_mutations.len()
        );
        Ok(())
    }
            ...
            for site_offset in 0..201 {
                ...
                for (ab_idx, &ab_ep) in self.antibody_epitope.iter().enumerate() {
                    if ab_ep == epitope_idx {
                        if let Some(&ef) =
                            self.raw_escape.get(ab_idx).and_then(|r| r.get(site_offset))
                        {
                            if ef > 0.0 {
                                sum += ef;
                                count += 1;
                ...
                self.escape_by_site[epitope_idx][site_offset] =
    ...
    pub fn get_ntd_escape(&self, lineage: &str) -> Option<f64> {
        ...
        eprintln!(
            "[WARNING] get_ntd_escape called for {} - NTD not in VASIL DMS data",
            lineage
        );
...
impl LineageMutations {
    ...
    pub fn load_from_vasil(vasil_data_dir: &Path, country: &str) -> Result<Self> {
        ...
        log::info!(
            "Loaded mutations for {} lineages",
            lineage_to_mutations.len()
        );
...
impl AllCountriesData {
    ...
    pub fn add_country(
        &mut self,
        name: &str,
        freq: GisaidFrequencies,
        mutations: LineageMutations,
        dms: DmsEscapeData,
    ) {
