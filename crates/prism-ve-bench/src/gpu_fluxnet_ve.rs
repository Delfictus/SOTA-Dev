...
impl GpuFluxNetVE {
    pub fn new(
        ...
        if !ptx_path.exists() {
            bail!(
                "PTX not found: {:?}. Compile with: nvcc -ptx mega_fused_vasil_fluxnet.cu",
                ptx_path
            );
        }
        ...
        let (base_ic50, base_thresholds) = if calibrated_path.exists() {
            if let Some((ic50, thresh)) = load_calibrated_params(calibrated_path) {
                eprintln!(
                    "[FluxNet] Loaded calibrated params from {:?}",
                    calibrated_path
                );
                (ic50, thresh)
            } else {
    ...
    fn initialize_configs(&mut self) -> Result<()> {
        ...
        for config_idx in 0..self.n_configs {
            ...
            for i in 0..N_IC50 {
                ic50_data
                    .push(self.best_ic50[i] * (1.0 + perturbation * (i as f32 / N_IC50 as f32)));
            }
        ...
        self.stream
            .memcpy_htod(&ic50_data, &mut self.d_ic50_configs)?;
        self.stream
            .memcpy_htod(&thresh_data, &mut self.d_threshold_configs)?;

        let init_accuracies = vec![self.baseline_accuracy; self.n_configs];
        self.stream
            .memcpy_htod(&init_accuracies, &mut self.d_prev_accuracies)?;
    ...
    pub fn train_step(
        ...
        let d_adjusted_gamma_min: CudaSlice<f64> =
            self.stream.alloc_zeros(self.n_configs * n_samples)?;
        let d_adjusted_gamma_max: CudaSlice<f64> =
            self.stream.alloc_zeros(self.n_configs * n_samples)?;
        let d_adjusted_gamma_mean: CudaSlice<f64> =
            self.stream.alloc_zeros(self.n_configs * n_samples)?;
        ...
        self.launch_gamma_adjust(
            ...
            d_base_gamma_max,
            d_base_gamma_mean,
            &d_adjusted_gamma_min,
            &d_adjusted_gamma_max,
            &d_adjusted_gamma_mean,
            n_samples,
        )?;

        let zeros_i32 = vec![0i32; self.n_configs];
        self.stream
            .memcpy_htod(&zeros_i32, &mut self.d_correct_counts)?;
        self.stream
            .memcpy_htod(&zeros_i32, &mut self.d_total_counts)?;
        self.stream
            .memcpy_htod(&zeros_i32, &mut self.d_excluded_counts)?;
        ...
        self.stream
            .memcpy_htod(&prev_accs, &mut self.d_prev_accuracies)?;
