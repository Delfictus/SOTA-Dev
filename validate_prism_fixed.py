# PRISM-Zero Validation Script for ChimeraX (Fixed)
...
def validate_prism(session):
    ...
    for i in range(min(20, len(displacements))):
        ...
        # We use 'id #5' to group all arrows
        cmd_str = f"shape cylinder start {p1[0]:.3f},{p1[1]:.3f},{p1[2]:.3f} end {p2[0]:.3f},{p2[1]:.3f},{p2[2]:.3f} radius 0.3 color yellow id #5"
        run(session, cmd_str)
