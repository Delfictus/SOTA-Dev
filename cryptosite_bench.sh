...
echo "============================================================" | tee "$LOG_FILE"
echo " CryptoSite CCNS Benchmark (PARALLEL -j8)" | tee -a "$LOG_FILE"
echo " Started: $(date)" | tee -a "$LOG_FILE"
echo " Results: $RESULTS_DIR" | tee -a "$LOG_FILE"
echo "============================================================" | tee -a "$LOG_FILE"

# Ground truth — assoc array for reference; also written as TSV for parallel
...
# Build TSV: PDB<tab>LABEL  — parallel can't receive assoc arrays
WORK_LIST="$RESULTS_DIR/work.tsv"
for PDB in "${!GROUND_TRUTH[@]}"; do
    echo -e "${PDB}\t${GROUND_TRUTH[$PDB]}"
done > "$WORK_LIST"

TOTAL=${#GROUND_TRUTH[@]}
echo "Total proteins: $TOTAL" | tee -a "$LOG_FILE"
...
run_protein() {
    ...
    echo "" >> "$LOG_FILE"
    echo "------------------------------------------------------------" >> "$LOG_FILE"
    echo "  $PDB (ground_truth=$LABEL) — $TIMESTAMP" >> "$LOG_FILE"
    echo "------------------------------------------------------------" >> "$LOG_FILE"
    echo "  START: $PDB (label=$LABEL)" | tee -a "$LOG_FILE"

    # Helper: write result JSON with flock
    write_result() {
        local STATUS="$1" TAU="$2" DG="$3" CLS="$4" SCORE="$5"
        (
            flock -x 200
            python3 -c "
import json, os
...
data['results'].append({
    'pdb': '$PDB', 'ground_truth': $LABEL, 'status': '$STATUS',
...
"
    ...
    if [ ! -f "$TOPOLOGY" ]; then
        ...
        write_result "missing_topology" "None" "None" "None" "None"
        return
    fi

    mkdir -p "$OUT_DIR"

    echo "  [$PDB] [1/2] nhs_rt_full ..." >> "$LOG_FILE"
    if RUST_LOG=info "$NHS_BIN" \
        ...
        --rt-clustering \
        --multi-scale \
        >> "$OUT_DIR/nhs.log" 2>&1; then
        echo "  [$PDB] NHS: OK" | tee -a "$LOG_FILE"
    else
        echo "  [$PDB] NHS FAILED" | tee -a "$LOG_FILE"
        write_result "nhs_failed" "None" "None" "None" "None"
        return
    fi
    ...
    if [ -z "$SPIKE_FILE" ]; then
        ...
        write_result "no_spikes" "None" "None" "None" "None"
        return
    fi

    local CCNS_OUT="$OUT_DIR/${PDB}.ccns.json"
    echo "  [$PDB] [2/2] ccns-analyze ..." >> "$LOG_FILE"
    if RUST_LOG=warn "$CCNS_BIN" \
        --input "$SPIKE_FILE" \
        --output "$CCNS_OUT" \
        >> "$OUT_DIR/ccns.log" 2>&1; then
        echo "  [$PDB] CCNS: OK" | tee -a "$LOG_FILE"
    else
        echo "  [$PDB] CCNS FAILED" | tee -a "$LOG_FILE"
        write_result "ccns_failed" "None" "None" "None" "None"
        return
    fi

    # Parse results and write with flock
    (
        flock -x 200
...
try:
    ...
    print(f"  DONE [{pdb}] [{'CORRECT' if correct else 'WRONG'}]: tau={tau:.4f}, class={cls}, dG={dg:.3f}, score={score:.3f} (GT={gt_str})")
except Exception as e:
    print(f"  PARSE ERROR [{pdb}]: {e}", file=sys.stderr)
    data = json.load(open(os.environ['RESULTS_JSON']))
    data['results'].append({'pdb': pdb, 'ground_truth': label, 'status': 'parse_error',
...
export -f run_protein

# ── Launch parallel ──────────────────────────────────────────────────────────
echo "Launching parallel --jobs 8 ..." | tee -a "$LOG_FILE"
parallel --jobs 8 --colsep '\t' run_protein {1} {2} :::: "$WORK_LIST"
